...
#define SYS_SOFTWARE_TIMER_STACK_SIZE		256
#define SYS_SOFTWARE_TIMER_TASK_QUEUE_MAX	64

INT32U SysSoftTimerStack[SYS_SOFTWARE_TIMER_STACK_SIZE];
void *software_tmr_task_q_stack[SYS_SOFTWARE_TIMER_TASK_QUEUE_MAX];
OS_EVENT *SysSoftTmrTaskQ = NULL;

// Functions defined in this file
void TimeBaseC_ISR(void);
INT32S sys_128Hz_timer_init(void);
void task_system_software_timer(void *para);

#define SYS_SOFTWARE_TIMER_TASK_PRIORITY    7

typedef enum
{
	TIMER0_END		= 0x00010000,
	TIMER1_END		= 0x00020000,
	TIMER2_END		= 0x00030000,
	TIMER3_END		= 0x00040000,
	TIMER4_END		= 0x00050000,
	TIMER5_END		= 0x00060000,
	TIMER6_END		= 0x00070000,
	TIMER7_END		= 0x00080000,
	TIMER8_END		= 0x00090000,
	TIMER9_END		= 0x000A0000,
	TIMER10_END		= 0x000B0000
}ENUM_SYS_SOFT_TIMER;
...
void sys_init_timer()
{
	...
	SysSoftTmrTaskQ = OSQCreate(software_tmr_task_q_stack, SYS_SOFTWARE_TIMER_TASK_QUEUE_MAX);
	OSTaskCreate(task_system_software_timer, (void *) 0, &SysSoftTimerStack[SYS_SOFTWARE_TIMER_STACK_SIZE - 1], SYS_SOFTWARE_TIMER_TASK_PRIORITY);
}
...
void task_system_software_timer(void *para)
{
	...
	while(1) {
		msg_id = (INT32U) OSQPend(SysSoftTmrTaskQ, 0, &err);
...
void TimeBaseC_ISR(void)
{
	...
	if(gstSysTimerEnable & TIMER0_ENABLE )
	{
		...
		if(gstSysTimer[0].CurrentTick == 0)
		{
			...
			//gstSysTimer[0].msg_qsend(gstSysTimer[0].msg_q_id, gstSysTimer[0].msg_id, NULL, 0, 0);
			OSQPost(SysSoftTmrTaskQ, (void *) TIMER0_END);
	...
	if(gstSysTimerEnable & TIMER1_ENABLE )
	{
		...
		if(gstSysTimer[1].CurrentTick == 0)
		{
			...
			//gstSysTimer[1].msg_qsend(gstSysTimer[1].msg_q_id, gstSysTimer[1].msg_id, NULL, 0, 0);
			OSQPost(SysSoftTmrTaskQ, (void *) TIMER1_END);
	...
	if(gstSysTimerEnable & TIMER2_ENABLE )
	{
		...
		if(gstSysTimer[2].CurrentTick == 0)
		{
			...
			//gstSysTimer[2].msg_qsend(gstSysTimer[2].msg_q_id, gstSysTimer[2].msg_id, NULL, 0, 0);
			OSQPost(SysSoftTmrTaskQ, (void *) TIMER2_END);
	...
	if(gstSysTimerEnable & TIMER3_ENABLE )
	{
		...
		if(gstSysTimer[3].CurrentTick == 0)
		{
			...
			//gstSysTimer[3].msg_qsend(gstSysTimer[3].msg_q_id, gstSysTimer[3].msg_id, NULL, 0, 0);
			OSQPost(SysSoftTmrTaskQ, (void *) TIMER3_END);
	...
	if(gstSysTimerEnable & TIMER4_ENABLE )
	{
		...
		if(gstSysTimer[4].CurrentTick == 0)
		{
			...
			//gstSysTimer[4].msg_qsend(gstSysTimer[4].msg_q_id, gstSysTimer[4].msg_id, NULL, 0, 0);
			OSQPost(SysSoftTmrTaskQ, (void *) TIMER4_END);
	...
	if(gstSysTimerEnable & TIMER5_ENABLE )
	{
		...
		{
			//gstSysTimer[5].msg_qsend(gstSysTimer[5].msg_q_id, gstSysTimer[5].msg_id, NULL, 0, 0);
			OSQPost(SysSoftTmrTaskQ, (void *) TIMER5_END);
	...
	if(gstSysTimerEnable & TIMER6_ENABLE )
	{
		...
		{
			//gstSysTimer[6].msg_qsend(gstSysTimer[6].msg_q_id, gstSysTimer[6].msg_id, NULL, 0, 0);
			OSQPost(SysSoftTmrTaskQ, (void *) TIMER6_END);
