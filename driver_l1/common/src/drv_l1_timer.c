...
static OS_EVENT		*sw_timer_sem = NULL;
#endif
...
#if _OPERATING_SYSTEM == 1	
	if(sw_timer_sem == NULL)
	{
		sw_timer_sem = OSSemCreate(1);
	}
#endif    
}

#if _OPERATING_SYSTEM == 1	
void sw_timer_lock(void)
{
	INT8U err;

	OSSemPend(sw_timer_sem, 0, &err);
}

void sw_timer_unlock(void)
{
	OSSemPost(sw_timer_sem);
}
#endif
...
INT32S timer_ccp_ctrl(INT32U TIMER_ID, INT32U PWMXSEL, INT32U CMPXSEL, INT32U CAPXSEL, INT32U CCPXEN)
{
    ...
    if (TIMER_ID < TIMER_E) {   /* only used for Timer A,B,C,D*/
        reg_value = (PWMXSEL <<0 | CMPXSEL<<4 | CAPXSEL<<8 | CCPXEN<<14);
    ...
    if (timer_id < TIMER_ID_MAX) {   /* only used for Timer A,B,C,D*/
        switch (timer_source)
        {
            ...
            case TIMER_SOURCE_32768HZ:
                timer_control(timer_id,TIMER_SOURCE_32768HZ,SRCB_1_,EXTBSEL_DEFAULT,EXTASEL_DEFAULT,
	                            TIMER_ENABLE, TIMER_INT_DISABLE, TIMER_INT_FLAG_FALSE);
                break; 
