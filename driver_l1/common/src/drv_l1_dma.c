...
static INT32U dma_get_channel(INT8U usage, INT32U t_addr, INT32U s_addr);
...
if (t_addr>0xF8000000) {
	for (i=C_DMA_CHANNEL_NUM-1; i>=0; i--) {

		if (t_addr == (INT32U)P_SPI0_TX_DATA)
		{
			if (dma_usage[FIXED_DMA0] != C_DMA_NOT_UESED)
			{
				DBG_PRINT("SPI0_TX busy\r\n");
				return C_DMA_CHANNEL_NUM;		// busy
			}
			
			dma_usage[FIXED_DMA0] = usage;
			...
			dma_set_control(FIXED_DMA0, C_DMA_CTRL_RESET);
			return FIXED_DMA0;
		}
		else if (s_addr==(INT32U)P_SPI0_RX_DATA)
		{
			if (dma_usage[FIXED_DMA1] != C_DMA_NOT_UESED)
			{
				DBG_PRINT("SPI0_RX busy\r\n");			
				return C_DMA_CHANNEL_NUM;		// busy
			}
		
			dma_usage[FIXED_DMA1] = usage;
			...
			// Reset DMA controller
			dma_set_control(FIXED_DMA1, C_DMA_CTRL_RESET);
			return FIXED_DMA1;
		}
		else if (s_addr==(INT32U)P_I2SRX_DATA)
		{
			if (dma_usage[FIXED_DMA7] != C_DMA_NOT_UESED)
			{
				DBG_PRINT("I2S_RX busy\r\n");						
				return C_DMA_CHANNEL_NUM;		// busy
			}

			dma_usage[FIXED_DMA7] = usage;
			...
			// Reset DMA controller
			dma_set_control(FIXED_DMA7, C_DMA_CTRL_RESET);
			return FIXED_DMA7;
		}

		// 若 AES 功能 disable，CH2 不能動（所以乾脆不用）
		if  ( (i==2)||(i==FIXED_DMA0)||(i==FIXED_DMA1)||(i==FIXED_DMA7) ) 
		{
			continue;
		}	
...
static INT32S dma_transfer_extend(DMA_STRUCT *dma_struct, INT8U usage, INT32U os_q)
{
	...
	channel = dma_get_channel(usage, t_addr, s_addr);
	...
	if ( (usage == C_DMA_OCCUPIED)&&(s_addr!=(INT32U)P_I2SRX_DATA) )		// ADC double buffer must equal lenght
	{
...
INT32S dma_memory_fill(INT32U t_addr, INT8U value, INT32U byte_count)
{
	...
	channel = dma_get_channel(C_DMA_NORMAL_USED,t_addr, t_addr);
	if  (channel >= C_DMA_CHANNEL_NUM) {
	  #if _OPERATING_SYSTEM != _OS_NONE
	  	dma_free_queue(q_index);
	  #endif
