...
static void cache_drain_write_buffer(void);
static INT32S cache_drain_line(INT32U bank, INT32U addr);
static INT32S cache_invalid_line(INT32U bank, INT32U addr);
static INT32S cache_drain_bank(INT32U bank);
static INT32S cache_invalid_bank(INT32U bank);

#if _OPERATING_SYSTEM == 1	
static OS_EVENT		*sw_cache_sem = NULL;

void sw_cache_lock(void)
{
	INT8U err;

	OSSemPend(sw_cache_sem, 0, &err);
}

void sw_cache_unlock(void)
{
	OSSemPost(sw_cache_sem);
}
#endif
...
#if _OPERATING_SYSTEM == 1	
	if(sw_cache_sem == NULL)
	{
		sw_cache_sem = OSSemCreate(1);
	}
#endif    
