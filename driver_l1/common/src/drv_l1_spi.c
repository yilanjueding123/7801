...
static OS_EVENT		*sw_spi_sem = NULL;

void sw_spi_lock(void)
{
	INT8U err;

	OSSemPend(sw_spi_sem, 0, &err);
}

void sw_spi_unlock(void)
{
	OSSemPost(sw_spi_sem);
}
#endif
...
static INT32S spi_transceive_dma(INT8U spi_num)
{
	DMA_STRUCT dma_struct_rx, dma_struct_tx;
	INT8S      done1,done2;
	INT32S     status;
	INT32S    retry = 0;
	INT32S    ret;
	...
	dma_struct_rx.s_addr = (INT32U) &spi_master[spi_num].spi_sfr->RX_DATA;
	dma_struct_rx.width = DMA_DATA_WIDTH_1BYTE;		// DMA_DATA_WIDTH_1BYTE or DMA_DATA_WIDTH_2BYTE or DMA_DATA_WIDTH_4BYTE
	if (spi_master[spi_num].rcv_len == 0) {
		dma_struct_rx.t_addr = (INT32U) 0xF8500600; /* reserved memory */
		dma_struct_rx.count = (INT32U) (spi_master[spi_num].len);
	}
	else {
		dma_struct_rx.t_addr = (INT32U) spi_master[spi_num].rx_buf;
		//dma_struct_rx.count = (INT32U) (spi_master[spi_num].rcv_len);
		dma_struct_rx.count = (INT32U) (spi_master[spi_num].len);
	}
	dma_struct_rx.notify = &done1;
	dma_struct_rx.timeout = 250;

	// SPI  DMA  TX
	done2 = C_DMA_STATUS_WAITING;
	dma_struct_tx.s_addr = (INT32U) spi_master[spi_num].tx_buf;
	dma_struct_tx.t_addr = (INT32U) &spi_master[spi_num].spi_sfr->TX_DATA;
	dma_struct_tx.width = DMA_DATA_WIDTH_1BYTE;		// DMA_DATA_WIDTH_1BYTE or DMA_DATA_WIDTH_2BYTE or DMA_DATA_WIDTH_4BYTE
	dma_struct_tx.count = (INT32U) (spi_master[spi_num].len);
	dma_struct_tx.notify = &done2;
	dma_struct_tx.timeout = 250;

	retry = 5;	// retrying number
	do {
		...
		status = dma_transfer(&dma_struct_rx);
		if (status != 0) {
			return status;
		}
		
		status = dma_transfer(&dma_struct_tx);
		if (status != 0) {
			return status;
		}
...
static INT32S spi_transceive_dma(INT8U spi_num)
{
	...
	INT32S    ret;
	INT32U    wifi_cnt = 0;
	INT32U   dma_burst8_len = (INT32U) (spi_master[spi_num].len&(~0x7));
	INT32U   dma_remainder_len = (INT32U) (spi_master[spi_num].len&(0x7));
	//INT32U   dma_brust8_len = (INT32U) (spi_master[spi_num].len);
	//INT32U   dma_remainder_len = 0;
	...
	if (spi_master[spi_num].rcv_len == 0) {
		dma_struct_rx.t_addr = (INT32U) 0xF8500600; /* reserved memory */
		dma_struct_rx.count = (INT32U) (dma_burst8_len);
	}
	else {
		dma_struct_rx.t_addr = (INT32U) spi_master[spi_num].rx_buf;
		//dma_struct_rx.count = (INT32U) (spi_master[spi_num].rcv_len);
	...
	dma_struct_tx.t_addr = (INT32U) &spi_master[spi_num].spi_sfr->TX_DATA;
	dma_struct_tx.width = DMA_DATA_WIDTH_1BYTE;		// DMA_DATA_WIDTH_1BYTE or DMA_DATA_WIDTH_2BYTE or DMA_DATA_WIDTH_4BYTE
	dma_struct_tx.count = (INT32U) (dma_burst8_len);
	dma_struct_tx.notify = &done2;
	dma_struct_tx.timeout = 250;
	...
	do {
		...
		status = dma_transfer_wait_ready(&dma_struct_tx);
		if (status != 0) {
			return status;
		}
		...
		while (done1 == C_DMA_STATUS_WAITING)
		{
			...
			{
				DBG_PRINT("DMA:miss\r\n");
				done1 = C_DMA_STATUS_TIMEOUT;
			}
		}
			
		if (done1 == C_DMA_STATUS_TIMEOUT || done2 == C_DMA_STATUS_TIMEOUT) {
			DBG_PRINT("done1=%d, done2=%d\r\n",done1,done2);
			DBG_PRINT("INT:0x%x\r\n",R_DMA_INT);
			DBG_PRINT("DMA:0x%x\r\n",R_DMA_DEVICE);
			ret =  SPI_TIMEOUT;
		}
	...
	if ( (retry!=0)&&(dma_remainder_len!=0) )
	{	
		dma_struct_rx.t_addr += dma_burst8_len;
		dma_struct_rx.count = dma_remainder_len;

		dma_struct_tx.s_addr += dma_burst8_len;
		dma_struct_tx.count = dma_remainder_len;
...
INT32S drv_l1_spi_cs_by_gpio_set(INT8U spi_num, INT8U enable, INT8U pin_num, INT8U active)
{
	...
	if(enable)
	{
		spi_cs_by_internal_set(spi_num,pin_num,active);

	}
	else
	{
		spi_cs_by_external_set(spi_num);
	}
