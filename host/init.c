...
{
    g_txrx_task_info[0].prio = OS_TX_TASK_PRIO;		//发送
    g_txrx_task_info[1].prio = OS_RX_TASK_PRIO;		//接收
...
ssv6xxx_result wifi_start(wifi_mode mode, bool static_ip, bool dhcp_server)
{
    ...
    ssv6xxx_result res;
    /* station mode */
	//__msg("mode = %d, static_ip = %d, dhcp_server = %d\n", mode, static_ip, dhcp_server);
...
int ssv6xxx_dev_init(ssv6xxx_hw_mode hmode)
{
	...
	g_log_module = CONFIG_LOG_MODULE;			//允许所有debug权限
    ...
    */
	...
	if ( (wifi_ssid[0]=='D')&&(wifi_ssid[1]=='B')&&(wifi_ssid[2]=='G') )
	{
  		uart_rx_enable(UART_USED_NUM);
		DBG_PRINT("Open Debug mode\r\n");
       	Cli_Init(0, NULL);
	}
#if (BUS_TEST_MODE == 0)
    ssv6xxx_wifi_cfg();
