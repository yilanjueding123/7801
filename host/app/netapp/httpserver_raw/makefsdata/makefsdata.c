...
int main(int argc, char *argv[])
{
  ...
  ssv6xxx_memset((void *)path, 0, sizeof(path));
  ssv6xxx_memset((void *)appPath, 0, sizeof(appPath));

  LOG_PRINTF(NEWLINE " makefsdata - HTML to C source converter" NEWLINE);
  LOG_PRINTF("     by Jim Pettinato               - circa 2003 " NEWLINE);
  LOG_PRINTF("     extended by Simon Goldschmidt  - 2009 " NEWLINE NEWLINE);
  ...
  if (!FINDFIRST_SUCCEEDED(fret)) {
    ...
    LOG_PRINTF(" Failed to open directory \"%s\"." NEWLINE NEWLINE, path);
    LOG_PRINTF(" Usage: htmlgen [targetdir] [-s] [-i] [-f:<filename>]" NEWLINE NEWLINE);
    LOG_PRINTF("   targetdir: relative or absolute path to files to convert" NEWLINE);
    LOG_PRINTF("   switch -s: toggle processing of subdirectories (default is on)" NEWLINE);
    LOG_PRINTF("   switch -e: exclude HTTP header from file (header is created at runtime, default is off)" NEWLINE);
    LOG_PRINTF("   switch -11: include HTTP 1.1 header (1.0 is default)" NEWLINE);
    LOG_PRINTF("   switch -c: precalculate checksums for all pages (default is off)" NEWLINE);
    LOG_PRINTF("   switch -f: target filename (default is \"fsdata.c\")" NEWLINE);
    LOG_PRINTF("   if targetdir not specified, htmlgen will attempt to" NEWLINE);
    LOG_PRINTF("   process files in subdirectory 'fs'" NEWLINE);
    exit(-1);
  }

  LOG_PRINTF("HTTP %sheader will %s statically included." NEWLINE,
    (includeHttpHeader ? (useHttp11 ? "1.1 " : "1.0 ") : ""),
    (includeHttpHeader ? "be" : "not be"));

  sprintf(curSubdir, "");  /* start off in web page's root directory - relative paths */
  LOG_PRINTF("  Processing all files in directory %s", path);
  if (processSubs) {
    LOG_PRINTF(" and subdirectories..." NEWLINE NEWLINE);
  } else {
    LOG_PRINTF("..." NEWLINE NEWLINE);
  }

  GETCWD(appPath, MAX_PATH_LEN);
  data_file = fopen("fsdata.tmp", "wb");
  if (data_file == NULL) {
    LOG_PRINTF("Failed to create file \"fsdata.tmp\"\n");
    exit(-1);
  }
  struct_file = fopen("fshdr.tmp", "wb");
  if (struct_file == NULL) {
    LOG_PRINTF("Failed to create file \"fshdr.tmp\"\n");
    exit(-1);
  }
  ...
  LOG_PRINTF(NEWLINE "Creating target file..." NEWLINE NEWLINE);
  ...
  LOG_PRINTF(NEWLINE "Processed %d files - done." NEWLINE NEWLINE, filesProcessed);
  ...
  if (processSubs) {
    ...
    if (FINDFIRST_SUCCEEDED(fret)) {
      do {
        ...
        LOG_PRINTF(NEWLINE "processing subdirectory %s/..." NEWLINE, curSubdir);
        filesProcessed += process_sub(data_file, struct_file);
        CHDIR("..");
  ...
  if (FINDFIRST_SUCCEEDED(fret)) {
    ...
    do {
      if (FIND_T_IS_FILE(fInfo)) {
        const char *curName = FIND_T_FILENAME(fInfo);
        LOG_PRINTF("processing %s/%s..." NEWLINE, curSubdir, curName);
                    ...
                    u16_t hdr_len, u16_t hdr_chksum)
{
  ...
  ssv6xxx_memset((void *)file_buffer_raw, 0xab, sizeof(file_buffer_raw));
  f = fopen(filename, "rb");
                           ...
                           u16_t *http_hdr_len, u16_t *http_hdr_chksum)
{
  ...
  ssv6xxx_memset((void *)hdr_buf, 0, sizeof(hdr_buf));
  ...
  if((file_ext == NULL) || (*file_ext == 0)) {
    LOG_PRINTF("failed to get extension for file \"%s\", using default.\n", filename);
  } else {
    ...
    if (j >= NUM_HTTP_HEADERS) {
      LOG_PRINTF("failed to get file type for extension \"%s\", using default.\n", file_ext);
  ...
  if (useHttp11) {
    ...
    ssv6xxx_memset((void *)intbuf, 0, sizeof(intbuf));
