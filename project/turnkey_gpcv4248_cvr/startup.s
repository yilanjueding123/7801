    ...
    IMPORT 	|Load$$ROM2$$Base|				; iRAM region
...
ROM_COPY_ADDR       EQU     0x597 ; ver B
;ROM_COPY_ADDR       EQU     0x785 ; ver C
...
21
	...
	CMP		R1, R2
	BEQ		%F23								; If load address is equal to execution address, movement is not needed
    CMP     R3, #0x0
    BLE     %F23
22
    LDR     R4, [R1], #4
...
Handle_FIQ
    ...
    CODE32
    LDR     R0, =system_power_on_ctrl;0xf8003e50
	MOV     LR, PC
	BX      R0
    LDR     R0,=ROM_COPY_ADDR;jump to rom code to move code
	BX      R0
	;MOV     R2,#0
    ;MOV     PC,R2
