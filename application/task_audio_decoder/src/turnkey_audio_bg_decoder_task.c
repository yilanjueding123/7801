...
INT32S g_auido_data_offset;	// added by Bruce, 2008/10/28
...
static void audio_bg_start(STAudioTaskPara *pAudioTaskPara)
{
	...
	if (audio_bg_context_p->source_type == AUDIO_SRC_TYPE_FS) {
		...
		ret = audio_play_file_set(pAudioTaskPara->fd, audio_bg_context_p, &audio_bg_ctrl);
	...
	else if (audio_bg_context_p->source_type == AUDIO_SRC_TYPE_USER_DEFINE)	// added by Bruce, 2008/10/27
	{
		...
		audio_bg_ctrl.file_len = g_audio_data_length;
		global_midi_index = pAudioTaskPara ->src_id;
		ret = AUDIO_ERR_NONE;
	}
   	else { /* resource file*/
...
INT32S SPU_read_a_sector_from_SDRAM(INT16S fd, INT32U data_offset_addr, INT32U buffer_addr)
{
	...
	{
		g_auido_data_offset = data_offset_addr;
		ret = audio_move_data(buffer_addr, 512);
	}
...
INT32S	SPU_load_tonecolor_from_SDRAM(INT16S fd_idi, INT32S MIDI_Index)
{
	...
	for(i=0; i<dwSize; i++)
	{
		...
		if( (offset_addr&0xfffffe00) != (offset_addr1&0xfffffe00) )//跨sector
		{
	...
	for(i=0; i<total_inst; i++)
	{
		...
		offset_addr = offset_addr1;//该句也可省略
	...
	midi_ring_buffer_ri = offset_addr & 0x000001ff;//表示从midi_ring_buffer的该位置开始播
	offset_addr1 = (offset_addr & 0xfffffe00) + 512;//指向下一个sector的开始
	U16_ptr = (INT16U*)(temp_buffer_addr + (offset_addr & 0x000001ff));
	U16_ptr1 = (INT16U*)(midi_ring_buffer_addr + (offset_addr & 0x000001ff));
	while((offset_addr < offset_addr1) && (midi_length))//将midi_ring_buffer中第一个sector以对齐的方式填满，前面可能有些空白
	{
		*U16_ptr1++ = *U16_ptr++;
		offset_addr += 2;
		midi_length -= 2;//表示可读的剩余midi长度
	}
	offset_addr = offset_addr1;//该句可省
